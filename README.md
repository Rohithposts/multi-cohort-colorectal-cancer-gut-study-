The aim of this study is to find patterns of taxa abundance levels at genus level that are consistently associated with the gut samples of patients affected with colorectal cancer when compared with healthy samples within the same group across multiple cohorts. Different cohorts often have different hidden confounders acting and affecting the reads generated (sequencing technology, geography, diet and so on). The goal is to find patterns of taxa at genus level whose abundunce level changes when compared to healthy samples are consistently being associated with gut of colorectal cancer across different cohorts having mutliple hidden confounders. 
...
For each of the output abundance table from each study, MaAslin2 analysis was done to obtain the list of genus whose abundance levels have changed and is statistically significant. Since Metaphlan's output abudnunce tables are relative abudnunces and the 16S QIIME2/R Workflow yields an abundance table with raw values, for the 16S tables, MaAslin2 was modelled with TSS Normalisation. The MaAslin2 output for each of the 4 cohorts can be accessed from the "MaAslin2_output_results" directory in this repository.
...
```
The analysis of the MaAslin2 output(s) is in "Analysis of the results.md" and the limitations of the study can be viewed at "Limitations_of_the_study.md".
